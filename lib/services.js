...
const createImportApiClient = () => {
  const authMiddlewareOptions = {
    credentials: {
      clientId: config.importClientId,
      clientSecret: config.importClientSecret
    },
    host: config.importAuthUrl,
    projectKey: config.importProjectKey,
    fetch
  }
 
  const httpMiddlewareOptions = {
    host: config.importApiUrl,
    fetch
  }
  ...
  return createImportApiBuilderFromCtpClient(
    client
  ).withProjectKeyValue({
    projectKey: config.importProjectKey
  })
